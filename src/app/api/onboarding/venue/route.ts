...
import { VenueOnboardingSchema } from '@/lib/validations/onboarding';
...
export async function POST(req: Request) {
  try {
    ...
    const result = VenueOnboardingSchema.safeParse(body);

    if (!result.success) {
      return NextResponse.json({ error: 'Invalid input', details: result.error.format() }, { status: 400 });
    }

    const { name, capacity } = result.data;
    ...
    logger.error({ err: error }, 'Venue Onboarding Error:');
